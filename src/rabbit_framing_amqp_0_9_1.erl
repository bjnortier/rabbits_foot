...
-type(amqp_method_field_name() ::
       ( insist | passive | queue | redelivered
       | requeue | frame_max | nowait | ticket
       | mechanism | active | method_id | global
       | no_ack | active | consumer_tag | message_count
       | arguments | delivery_tag | ticket | durable
       | channel_max | capabilities | arguments | exchange
       | delivery_tag | queue | ticket | message_count
       | queue | reply_code | delivery_tag | exchange
       | nowait | exchange | delivery_tag | consumer_count
       | known_hosts | routing_key | frame_max | ticket
       | exclusive | nowait | type | active
       | requeue | version_major | response | nowait
       | nowait | arguments | ticket | arguments
       | auto_delete | if_unused | queue | heartbeat
       | internal | consumer_tag | exchange | reply_text
       | multiple | locale | redelivered | response
       | consumer_tag | version_minor | exchange | routing_key
       | ticket | out_of_band | nowait | routing_key
       | reply_code | ticket | immediate | cluster_id
       | consumer_tag | prefetch_count | no_ack | queue
       | ticket | ticket | prefetch_size | if_empty
       | realm | routing_key | reply_code | ticket
       | routing_key | exchange | ticket | mechanisms
       | requeue | reply_text | virtual_host | write
       | exclusive | routing_key | passive | heartbeat
       | durable | auto_delete | arguments | nowait
       | message_count | exclusive | client_properties | reply_text
       | channel_max | channel_id | class_id | server_properties
       | queue | consumer_tag | no_local | if_unused
       | message_count | exchange | queue | challenge
       | locales | mandatory | passive | nowait
       | class_id | read | exchange | queue
       | method_id )).
